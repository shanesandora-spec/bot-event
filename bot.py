    ...
    def __init__(self, guild=None):
        options = [
            discord.SelectOption(
                ...
                emoji=discord.utils.get(guild.emojis, name="mirage") if guild else None
            ),
            discord.SelectOption(
                label="Капт", 
                description="Захват территории для банд (5-60 мин)", 
                emoji=discord.utils.get(guild.emojis, name="kapt") if guild else None
            ),
            discord.SelectOption(
                label="Битва за притон", 
                description="Сражение за наркопритон (до 24ч)", 
                emoji=discord.utils.get(guild.emojis, name="bitva_za_priton") if guild else None
            ),
            discord.SelectOption(
                label="Битва за нефтевышки", 
                description="Борьба за нефтяные вышки (до 24ч)", 
                emoji=discord.utils.get(guild.emojis, name="bitva_za_neftevishky") if guild else None
            ),
            discord.SelectOption(
                label="Семейные битвы", 
                description="Масштабные войны семей (до 24ч)", 
                emoji=discord.utils.get(guild.emojis, name="semeynaya_bitva") if guild else None
            ),
        ]
        
        super().__init__(placeholder="✨ Выберите мероприятие для создания оповещения...", min_values=1, max_values=1, options=options)
...
def get_timestamp(minutes_str):
    try:
        mins = int(''.join(filter(str.isdigit, minutes_str))) if not minutes_str.isdigit() else int(minutes_str)
    except:
        mins = 5
...
# Оранжево-красный цвет плашки
EMBED_COLOR = discord.Color.from_rgb(255, 87, 34)


# --- 1. МИРАЖ ---
class MirageModal(ui.Modal, title="✈️ Операция «Мираж»"):
    server_input = ui.TextInput(label="Номер сервера", placeholder="Например: 24", style=discord.TextStyle.short, required=True)
    time_input = ui.TextInput(label="Минут до начала (число)", placeholder="Например: 5", style=discord.TextStyle.short, required=True)
    ...
    async def on_submit(self, interaction: discord.Interaction):
        ...
        embed.set_footer(text="Arizona RP ⚔️ Event Prime", icon_url=interaction.user.display_avatar.url)

        if channel:
            await channel.send(content=role_mentions, embed=embed)
            await interaction.response.send_message("✅ Оповещение опубликовано!", ephemeral=True)
        else:
...
class CaptModal(ui.Modal, title="🔫 Захват территории (Капт)"):
    server_input = ui.TextInput(label="Номер сервера", placeholder="Например: 24", style=discord.TextStyle.short, required=True)
    band_input = ui.TextInput(label="Банда", placeholder="Groove, Ballas, Vagos...", style=discord.TextStyle.short, required=True)
    time_input = ui.TextInput(label="Минут до начала (число)", placeholder="Например: 10", style=discord.TextStyle.short, required=True)
...
class PritonModal(ui.Modal, title="💊 Битва за притон"):
    server_input = ui.TextInput(label="Номер сервера", placeholder="Например: 24", style=discord.TextStyle.short, required=True)
    band_input = ui.TextInput(label="Банда", placeholder="За кого заходим", style=discord.TextStyle.short, required=True)
    time_input = ui.TextInput(label="Минут до начала (число)", placeholder="Например: 15", style=discord.TextStyle.short, required=True)
...
class OilModal(ui.Modal, title="🛢️ Битва за нефтевышки"):
    server_input = ui.TextInput(label="Номер сервера", placeholder="Например: 24", style=discord.TextStyle.short, required=True)
    band_input = ui.TextInput(label="За кого заходим", placeholder="Мафия / Семья / Организация", style=discord.TextStyle.short, required=True)
    time_input = ui.TextInput(label="Минут до начала (число)", placeholder="Например: 20", style=discord.TextStyle.short, required=True)
...
class FamilyModal(ui.Modal, title="👑 Семейные битвы"):
    server_input = ui.TextInput(label="Номер сервера", placeholder="Например: 24", style=discord.TextStyle.short, required=True)
    family_input = ui.TextInput(label="Название семьи", placeholder="Введите название", style=discord.TextStyle.short, required=True)
    phone_input = ui.TextInput(label="Связь", placeholder="Тег лидера / связь", style=discord.TextStyle.short, required=True)
    time_input = ui.TextInput(label="Минут до начала (число)", placeholder="Например: 15", style=discord.TextStyle.short, required=True)
...
class MyBot(discord.Client):
    ...
    async def setup_hook(self):
        @self.tree.command(name="mpevent", description="Панель управления оповещениями мероприятий")
